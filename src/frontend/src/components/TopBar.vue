<template>
  <header class="h-16 flex-shrink-0 bg-surface-container-lowest/85 border-b border-outline-variant shadow-sm flex justify-between items-center px-md md:px-lg z-50 backdrop-blur-xl">
    <!-- Left -->
    <div class="flex items-center gap-lg">
      <div class="w-9 h-9 rounded-lg bg-primary text-on-primary flex items-center justify-center font-black md:hidden">PM</div>
      <div class="font-headline-md text-headline-md font-black text-on-surface hidden md:block">Quản Lý Dự Án</div>
    </div>

    <!-- Right -->
    <div class="flex items-center gap-sm">
      <!-- Search -->
      <div class="hidden md:flex items-center app-input rounded-full px-3 py-1.5 w-56">
      ...
      <button @click="toggleTheme"
        class="text-on-surface-variant hover:bg-surface-container-high hover:text-on-surface rounded-full p-2 transition-all active:scale-95"
      ...
      <div class="relative">
        <button @click="toggleNotif"
          class="relative text-on-surface-variant hover:bg-surface-container-high hover:text-on-surface rounded-full p-2 transition-all active:opacity-80">
        ...
        <div v-if="showNotif" @click.stop
          class="absolute right-0 top-full mt-2 w-80 bg-surface-container-lowest rounded-xl shadow-xl border border-outline-variant z-[60] overflow-hidden">
      ...
      <div class="relative">
        <button @click="showMenu = !showMenu"
          class="flex items-center gap-2 rounded-full pl-1 pr-2 py-1 hover:bg-surface-container-high transition-colors">
        ...
        <div v-if="showMenu" @click.stop
          class="absolute right-0 top-full mt-2 w-52 bg-surface-container-lowest rounded-xl shadow-xl border border-outline-variant py-1 z-[60]">
...
async function toggleNotif() {
  ...
  if (showNotif.value) {
    try {
      ...
      unreadCount.value = all.filter(n => !n.isRead).length
    } catch { /* ignore */ }
  }
}
