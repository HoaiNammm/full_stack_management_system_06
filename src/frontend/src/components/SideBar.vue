<template>
  <nav
    class="hidden md:flex w-[280px] h-screen fixed left-0 top-0 bg-surface-container-lowest text-on-surface border-r border-outline-variant shadow-xl flex-col px-md py-lg z-40 overflow-hidden"
  >
    <RouterLink to="/dashboard" class="flex items-center gap-sm mb-lg px-xs group flex-shrink-0">
      <div
        class="w-11 h-11 rounded-xl bg-primary text-on-primary flex items-center justify-center font-black shadow-sm group-hover:scale-105 transition-transform"
      >
        PM
      </div>
      <div class="min-w-0">
    ...
    <div class="flex-1 min-h-0 overflow-y-auto overflow-x-hidden pr-1">
      <div class="flex flex-col gap-1">
        <p class="px-sm pb-xs font-label-sm text-label-sm text-outline uppercase tracking-wider">
          Main menu
        </p>
        ...
        >
          ...
          <span
            ...
            :style="{ fontVariationSettings: isActive(item) ? `'FILL' 1` : `'FILL' 0` }"
          >
            {{ item.icon }}
          </span>
        ...
        <p
          class="px-sm pt-lg pb-xs font-label-sm text-label-sm text-outline uppercase tracking-wider"
        >
          Workspace
        </p>
    ...
    <div class="flex-shrink-0 pt-md">
      <div class="rounded-2xl border border-outline-variant bg-surface-container-low p-sm mb-sm">
        <p class="font-label-sm text-label-sm text-on-surface-variant leading-5">Demo workspace</p>
        <p class="font-label-lg text-label-lg text-on-surface leading-6">FE + 3 services</p>
      </div>

      <button @click="$router.push('/projects')" class="app-button-primary w-full">
