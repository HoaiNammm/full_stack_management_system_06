<template>
  <div class="flex flex-col h-full overflow-hidden">
    ...
    <div
      v-if="loading"
      class="flex-1 flex items-center justify-center text-on-surface-variant gap-2"
    >
    ...
    <div v-else-if="error" class="flex-1 flex items-center justify-center">
      <div
        class="bg-error-container/20 border border-error/30 rounded-xl p-lg text-error font-label-md flex flex-col items-center gap-3"
      >
    ...
    <template v-else-if="project">
      <!-- Page Header -->
      <div
        class="px-lg pt-6 pb-0 border-b border-outline-variant bg-surface-container-lowest/85 flex-shrink-0 backdrop-blur-xl"
      >
        <div class="max-w-7xl mx-auto">
          ...
          <div class="flex items-center gap-2 mb-4">
            ...
            <span class="font-label-md text-label-md text-on-surface truncate max-w-[200px]">{{
              project.name
            }}</span>
          </div>

          <!-- Project info -->
          <div class="gradient-panel p-lg mb-4 overflow-hidden">
            <img
              :src="projectCover"
            ...
            />
            <div
              class="absolute inset-0 bg-gradient-to-r from-surface/95 via-surface/80 to-surface/30"
            ></div>
            <div class="relative z-10 flex items-start gap-md">
              <div
                class="w-12 h-12 rounded-xl flex items-center justify-center font-bold text-white font-headline-sm flex-shrink-0"
                :style="{ backgroundColor: project.color || '#3525cd' }"
              >
      ...
      <div class="flex-1 overflow-y-auto">
        <div class="max-w-7xl mx-auto px-lg py-lg">
          ...
          <div
            ...
            class="grid grid-cols-1 xl:grid-cols-[1fr_340px] gap-lg"
          >
            <div class="flex flex-col gap-md">
              ...
              <div class="grid grid-cols-2 sm:grid-cols-4 gap-md">
                ...
                >
                  ...
                  <div class="flex items-center gap-2">
                    <span class="material-symbols-outlined text-[20px]" :class="stat.iconColor">{{
                      stat.icon
                    }}</span>
                    <span class="font-headline-sm text-headline-sm" :class="stat.color">{{
                      stat.value
                    }}</span>
                  </div>
                </div>
              ...
              <div class="grid grid-cols-1 lg:grid-cols-2 gap-md">
                <div class="app-panel p-md">
                  <div class="flex items-start justify-between gap-3">
                    ...
                    <span class="soft-badge" :class="projectHealth.badge">{{
                      projectHealth.label
                    }}</span>
                  </div>
                ...
                <div class="app-panel p-md">
                  <div class="flex items-start justify-between gap-3">
                    <div>
                      ...
                      <h3 class="font-headline-sm text-headline-sm text-on-surface">
                        {{ activeSprint?.name || 'Chưa có sprint đang chạy' }}
                      </h3>
                    </div>
                  ...
                  <p class="font-body-md text-body-md text-on-surface-variant mt-2">
                    {{
                      activeSprint?.goal ||
                      'Bắt đầu sprint để theo dõi nhịp phát triển và milestone.'
                    }}
                  </p>
                  <button @click="activeTab = 'sprints'" class="app-button-secondary mt-md">
                    Xem sprint
                  </button>
                </div>
              </div>
              ...
              <div class="app-panel">
                <div
                  class="px-md py-sm border-b border-outline-variant flex items-center justify-between"
                >
                  <div>
                    <h3 class="font-headline-sm text-headline-sm text-on-surface">
                      Upcoming deadlines
                    ...
                    <p class="font-body-md text-body-md text-on-surface-variant">
                      Task sắp tới và task cần chú ý.
                    </p>
                  </div>
                  <button
                ...
                <div class="p-md grid grid-cols-1 md:grid-cols-2 gap-sm">
                  <div v-for="task in upcomingTasks" :key="task.id" class="data-row">
                    <p class="font-label-lg text-label-lg text-on-surface truncate">
                      {{ task.title }}
                    </p>
                  ...
                  <div
                    ...
                    class="text-on-surface-variant font-body-md text-body-md"
                  >
            ...
            <div class="flex flex-col gap-md">
              <div class="app-panel p-md">
                ...
                <div class="flex flex-col gap-2">
                  <div v-for="m in members.slice(0, 6)" :key="m.id" class="flex items-center gap-2">
                    ...
                    <span class="font-label-lg text-label-lg text-on-surface flex-1 truncate">{{
                      m.displayName
                    }}</span>
                    <span
                      class="text-[10px] font-bold px-1.5 py-0.5 rounded-full"
                      :class="roleStyle(m.role)"
                      >{{ roleName(m.role) }}</span
                    >
                  </div>
                </div>
              </div>
              ...
              <div class="app-panel">
                ...
                <div class="p-md flex flex-col gap-sm">
                  <div
                    v-for="log in activities.slice(0, 4)"
                    :key="log.id"
                    class="data-row flex items-start gap-3"
                  >
                    <div
          ...
          <div v-if="activeTab === 'timeline'" class="flex flex-col gap-md">
            <div class="gradient-panel p-lg overflow-hidden">
              <div class="relative z-10 flex items-center justify-between gap-3 mb-6">
                ...
                <button
                  @click="$router.push({ path: '/calendar', query: { projectId: project.id } })"
              ...
              <div class="relative z-10 flex flex-col gap-3">
                <div
                  ...
                  :key="item.id"
                  class="grid grid-cols-[92px_1fr_auto] md:grid-cols-[140px_1fr_auto] gap-3 items-center"
                >
                  <span class="font-label-md text-label-md text-on-surface-variant">{{
                    formatDate(item.date)
                  }}</span>
                  <div
                    class="h-12 rounded-2xl border border-outline-variant bg-surface-container-low flex items-center overflow-hidden shadow-sm"
                  >
                ...
                <div
                  ...
                  class="text-center py-xl text-on-surface-variant"
                >
          ...
          <div v-if="activeTab === 'kanban'" class="app-panel p-lg flex flex-col gap-lg">
            <div class="flex items-center justify-between gap-3">
              ...
              <button
                @click="$router.push({ path: '/kanban', query: { projectId: project.id } })"
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-md">
              <div v-for="summary in kanbanSummary" :key="summary.label" class="workspace-card">
                <p
                  class="font-label-md text-label-md uppercase tracking-wider text-on-surface-variant"
                >
                  {{ summary.label }}
                </p>
                <div class="mt-3 flex items-end justify-between gap-3">
                  ...
                  <div
                    class="h-2 flex-1 rounded-full bg-surface-container-high overflow-hidden max-w-[120px]"
                  >
                    <div
                      class="h-full rounded-full bg-primary"
                      :style="{ width: Math.min(100, summary.value * 18 + 8) + '%' }"
                    ></div>
                  </div>
                </div>
              </div>
          ...
          <div v-if="activeTab === 'activity'" class="app-panel">
            <div class="px-md py-sm border-b border-outline-variant">
              ...
              <p class="font-body-md text-body-md text-on-surface-variant">
                Hoạt động được ghi nhận từ NotifyService.
              </p>
            </div>
            <div
              v-if="activityLoading"
              class="p-lg flex items-center gap-2 text-on-surface-variant"
            >
            ...
            <div
              ...
              class="p-lg text-center text-on-surface-variant"
            >
            ...
            <div v-else class="divide-y divide-outline-variant/50">
              <div
                ...
                :key="log.id"
                class="px-md py-sm flex items-start gap-3"
              >
                <div
                  class="w-8 h-8 rounded-full bg-primary/10 text-primary flex items-center justify-center flex-shrink-0"
                >
          ...
          <!-- Comments Tab -->
          <div v-if="activeTab === 'comments'" class="app-panel p-lg">
            <div class="grid grid-cols-1 lg:grid-cols-[1fr_340px] gap-lg">
              <div>
                ...
                <p class="font-body-md text-body-md text-on-surface-variant mt-2">
                  Khu vực trao đổi nội bộ của dự án, hỗ trợ bình luận, mention thành viên và theo
                  dõi hoạt động mới nhất.
                </p>

                <div class="mt-md flex gap-3">
                  <input
                    v-model="newComment"
                    class="flex-1 rounded-2xl border border-outline-variant bg-surface px-4 py-3 text-on-surface outline-none"
                    placeholder="Nhập bình luận cho dự án..."
                    @keyup.enter="addProjectComment"
                  />
                  <button @click="addProjectComment" class="app-button-primary">Gửi</button>
                </div>
                ...
                <div class="mt-lg flex flex-col gap-md">
                  <div
                    ...
                    :key="comment.id"
                    class="workspace-card flex items-start gap-3"
                  >
                    <div
                    ...
                    <div class="min-w-0">
                      <div class="flex items-center gap-2">
                        ...
                        <span class="text-on-surface-variant text-sm">{{ comment.time }}</span>
                      </div>
                      <p class="font-body-md text-body-md text-on-surface-variant mt-1">
                        {{ comment.content }}
                      </p>
              ...
              <div class="glass-card p-md">
                <div class="flex items-center gap-3 mb-md">
                  ...
                  <div>
                    <p class="font-label-lg text-label-lg text-on-surface">Hoạt động gần đây</p>
                    <p class="font-label-sm text-label-sm text-on-surface-variant">
                      Tổng hợp từ Comment & Notify Service.
                    </p>
                  </div>
                </div>
                ...
                <div class="flex flex-col gap-sm">
                  <div
                    v-for="item in recentDiscussionActivities"
                    :key="item.id"
                    class="rounded-2xl bg-surface-container px-3 py-2"
                  >
...
const tabs = [
  ...
  { id: 'comments', label: 'Thảo luận', icon: 'forum' },
]
...
const currentMember = computed(() =>
  members.value.find(
    (member) => String(member.userId).toLowerCase() === String(user.value?.id || '').toLowerCase(),
  ),
)
...
const projectCover = computed(() => getProjectCover(project.value || {}, 0))
...
const overviewStats = computed(() => [
  {
    label: 'Trạng thái',
    value: project.value?.status === 1 ? 'Active' : 'Draft',
...
const doneTaskCount = computed(
  () => projectTasks.value.filter((t) => t.columnType === 'done').length,
)
...
const riskyTasks = computed(() =>
  projectTasks.value.filter(
    (t) => t.dueDate && new Date(t.dueDate) < new Date() && t.columnType !== 'done',
  ),
)
...
const projectHealth = computed(() => {
  if (riskyTasks.value.length > 0)
    return { label: 'At risk', badge: 'bg-error-container/40 text-error' }
  if (projectProgress.value >= 70)
    return { label: 'Healthy', badge: 'bg-secondary/10 text-secondary' }
...
const timelineItems = computed(() => {
  ...
  for (const sprint of sprints.value) {
    items.push({
      ...
      color:
        sprint.status === 1 ? 'bg-primary' : sprint.status === 2 ? 'bg-secondary' : 'bg-outline',
      badgeClass:
        sprint.status === 1
          ? 'bg-primary/10 text-primary'
          : sprint.status === 2
...
const kanbanSummary = computed(() => {
  ...
  const overdue = projectTasks.value.filter(
    (t) => t.dueDate && new Date(t.dueDate) < new Date() && t.columnType !== 'done',
  ).length
  return [
...
async function loadDetail() {
  ...
  if (!id) {
    error.value = 'Không tìm thấy ID dự án'
    loading.value = false
    return
  }
