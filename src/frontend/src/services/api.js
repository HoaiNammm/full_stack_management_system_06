...
export const projectService = {
  ...
  createSprint: (pid, body) =>
    projectApi.post(`/projects/${pid}/sprints`, body).then((r) => r.data.data),
  updateSprint: (pid, sid, body) =>
    projectApi.put(`/projects/${pid}/sprints/${sid}`, body).then((r) => r.data.data),
  ...
  createMilestone: (pid, body) =>
    projectApi.post(`/projects/${pid}/milestones`, body).then((r) => r.data.data),

  getMembers: (pid) => projectApi.get(`/projects/${pid}/members`).then((r) => r.data.data),
  addMember: (pid, body) =>
    projectApi.post(`/projects/${pid}/members`, body).then((r) => r.data.data),
  updateMemberRole: (pid, mid, body) =>
    projectApi.put(`/projects/${pid}/members/${mid}/role`, body).then((r) => r.data.data),
...
export const taskService = {
  ...
  getColumns: (projectId) => {
    ...
    return taskApi
      .get('/kanban-columns', { params: { projectId } })
...
export const notifyService = {
  getAll: (unreadOnly = false) =>
    notifyApi.get('/notifications', { params: { unreadOnly } }).then((r) => r.data.data),
  unreadCount: () =>
    notifyApi.get('/notifications/unread-count').then((r) => r.data.data?.count ?? 0),
  markRead: (id) => notifyApi.put(`/notifications/${id}/read`),
  markAllRead: () => notifyApi.put('/notifications/read-all'),
  getProjectActivity: (projectId) =>
    notifyApi.get(`/activity-logs/project/${projectId}`).then((r) => r.data.data ?? r.data),
  getTaskActivity: (taskId) =>
    notifyApi.get(`/activity-logs/task/${taskId}`).then((r) => r.data.data ?? r.data),
  getProjectComments: (projectId) =>
    notifyApi.get(`/comment-workflows/project/${projectId}`).then((r) => r.data.data),

  createProjectComment: (projectId, body) =>
    notifyApi.post(`/comment-workflows/project/${projectId}`, body).then((r) => r.data.data),
}
